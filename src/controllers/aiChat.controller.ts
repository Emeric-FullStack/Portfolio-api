    ...
    messages: [
      {
        role: 'system',
        content:
          "Tu es un assistant spécialisé dans la génération de code propre et bien formaté. Tu as accès à l'historique complet de la conversation et tu dois l'utiliser pour maintenir le contexte. Format des réponses : utiliser des blocs Markdown(```language\\ncode```) pour le code et '```plaintext' pour le texte.",
      },
...
const handleDeepseekPrompt = async (
  apiKey: string,
  messages: Array<{ role: 'user' | 'assistant' | 'system'; content: string }>,
) => {
