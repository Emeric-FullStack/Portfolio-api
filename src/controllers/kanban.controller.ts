...
export const getUserBoards: RequestHandler = async (req, res): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: "Utilisateur non authentifié" });
            return;
...
export const addMember: RequestHandler = async (req, res): Promise<void> => {
    try {
        ...
        if (board.members.includes(member._id as unknown as Schema.Types.ObjectId)) {
...
export const updateListPosition: RequestHandler = async (req, res): Promise<void> => {
    try {
        const { boardId, listId } = req.params;
        const { position: newPosition } = req.body;

        // 1. Récupérer toutes les listes du board, triées par position
        let lists = await List.find({ board: boardId }).sort('position');
        
        // 2. Trouver la liste déplacée
        ...
        // 3. Retirer la liste de sa position actuelle
        lists = lists.filter(list => list._id.toString() !== listId);

        // 4. Réinsérer la liste à sa nouvelle position
        lists.splice(newPosition, 0, draggedList);

        // 5. Réajuster toutes les positions séquentiellement
        ...
        // 7. Retourner les listes mises à jour avec leurs cartes
...
export const createCard: RequestHandler = async (req, res): Promise<void> => {
    try {
        ...
        if (!title || !listId || !boardId) {
            res.status(400).json({ message: "Titre, ID de la liste et du tableau requis" });
            return;
        }
...
export const updateCard: RequestHandler = async (req, res): Promise<void> => {
    try {
        ...
        const { title, listId, boardId, position, description, priority } = req.body;
...
export const updateCardPosition: RequestHandler = async (req, res): Promise<void> => {
    try {
...
export const createActivity: RequestHandler = async (req, res): Promise<void> => {
    try {
...
export const getActivitiesByBoardId: RequestHandler = async (req, res): Promise<void> => {
    try {
        const boardId = req.params.id;
...
export const getActivitiesByCardId: RequestHandler = async (req, res): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ message: "Utilisateur non authentifié" });
            return;
...
export const getCommentsByCardId: RequestHandler = async (req, res): Promise<void> => {
    try {
        const cardId = req.params.id;
...
export const createChecklist: RequestHandler = async (req, res): Promise<void> => {
    try {
...
export const getChecklistsByCardId: RequestHandler = async (req, res): Promise<void> => {
    try {
        const cardId = req.params.id;
...
export const updateChecklist: RequestHandler = async (req, res): Promise<void> => {
    try {
...
export const deleteChecklist: RequestHandler = async (req, res): Promise<void> => {
    try {
        const checklistId = req.params.id;
        const checklist = await Checklist.findById(checklistId);
        if (!checklist) {
...
export const createChecklistItem: RequestHandler = async (req, res): Promise<void> => {
    try {
...
export const deleteChecklistItem: RequestHandler = async (req, res): Promise<void> => {
    try {
...
export const toggleChecklistItem: RequestHandler = async (req, res): Promise<void> => {
    try {
        const { checklistId, itemId } = req.params;
...
export const moveCardToList: RequestHandler = async (req, res): Promise<void> => {
    try {
        const { cardId } = req.params;
        ...
        const cardsInNewList = await Card.find({ listId: newListId }).sort('position');
